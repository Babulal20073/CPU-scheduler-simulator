...
$(document).ready(function () {
	...
	function SJF() {
		...
		while (isDone() == false) {
			...
			bar.addItem(processArray[i].processName, processArray[i].burstTime);
			position += processArray[i].burstTime;  // Move the clock forward
			processArray[i].completionTime = position;  // Store completion time
			processArray[i].finished();
	...
	function run() {
		...
		if (processArray.length > 0) {
			...
			if (algorithm == "FCFS") {
				$("#algorithm_explanation").html("The process that arrives first gets the CPU first, executing in the order of arrival.<br>Decision Criteria: Arrival time (earliest first)");
				// $("#algorithm_explanation").text("Decision Criteria: Arrival time (earliest first)")
				FCFS();
				processTotal = processArray;
			}

			else if (algorithm == "SJF") {
				$("#algorithm_explanation").html("The process that needs the least CPU time will be executed first.<br>Decision Criteria: Burst time (shortest first)");
				SJF();
				processTotal = processArray;
			}
				...
				SRJF();
				$("#algorithm_explanation").html("The process with the shortest remaining time will always execute first, preempting the current process if needed.<br>Decision Criteria: Remaining burst time (shortest first)");
				processTotal = processArray;
			}

			else if (algorithm == "Round Robin") {
				$("#algorithm_explanation").html("Each process gets a fixed time slice (quantum) and is preempted if its time is up, moving to the back of the queue.<br>Decision Criteria: Time quantum (fixed), Arrival time (round-robin order)");
				roundRobin();
				processTotal = processArray;
				tq = timeQuantum;
			}
			...
			if (algorithm == "Priority") {
				...
				$("#algorithm_explanation").html("The process with the highest priority gets the CPU first, but if a higher-priority process arrives later, it preempts the current one.<br>Decision Criteria: Priority (higher first), Preemption allowed");
				priority();
				processTotal = processArray;
			}
...
$(".runButton").click(function () {
	...
	for (let i = 0; i < totalProcess; i++) {
		...
		// mainOutput.waitingTime[i] = (processTotal[i].turnAroundTime - processTotal[i].burstTime);
